...
var Connection = function(config) {
  ...
  config = config || {};
  this.stream = config.stream || (isNode ? new net.Stream() : new netStreamEmulation());
  if (!this.stream.getReadyState) {
    if (!isNode) throw new Error("what's going on? not using emulation, not using net.stream? constructor given stream outside node.js? not supported");
